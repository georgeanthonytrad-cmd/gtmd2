...
def get_yfinance_info(symbol):
    ...
    Your original app worked mainly from yfinance's info dictionary.
    Some environments return blanks from get_info(), so this tries:
    1) ticker.get_info()
    2) ticker.info
    3) ticker.fast_info for price fallback fields
    """
    if yf is None:
        return {}
    ...
    try:
        ...
        # Method 1: newer yfinance method
        try:
        ...
        # Method 2: older/original yfinance property
        try:
            data = ticker.info
            if isinstance(data, dict):
        ...
        # Method 3: fast_info price fallback
        try:
...
def build_metrics(symbol):
    ...
    short_float_raw = first_valid(
        ...
        yf_info.get("sharesShortPriorMonth"),
        nested_raw(yahoo_summary, "defaultKeyStatistics", "shortPercentOfFloat"),
    )

    # If only raw shares short is available, convert to % of float when possible.
    if is_num(short_float_raw) and safe_float(short_float_raw) > 1:
        shares_float = first_valid(
            yf_info.get("floatShares"),
            nested_raw(yahoo_summary, "defaultKeyStatistics", "floatShares"),
        )
        if is_num(shares_float) and shares_float > 0:
            short_float_raw = safe_float(short_float_raw) / shares_float
...
if mode == "Single Stock Analyzer":
    ...
    if run and symbol:
        ...
        if not is_num(safe_float(values["Current Price"])):
            ...
            st.caption(f"Fundamental data status: {values.get('Fundamental Data Status', 'Unknown')}")

        with st.expander("Data source diagnostics"):
            for item in debug:
                st.write("-", item)
            if yf_info := get_yfinance_info(symbol):
                visible_keys = [
                    "trailingPE", "forwardPE", "pegRatio", "trailingPegRatio",
                    "earningsQuarterlyGrowth", "earningsGrowth", "revenueGrowth",
                    "targetMeanPrice", "shortPercentOfFloat", "totalCash"
                ]
                st.write("Available yfinance keys checked:")
                st.write({k: yf_info.get(k) for k in visible_keys if k in yf_info})
