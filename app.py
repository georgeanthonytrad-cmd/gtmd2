...
# SETUP
# -----------------------------
st.set_page_config(page_title="Yahoo Stock Scoring Dashboard", layout="wide")
...
def get_yahoo_chart_history(symbol):
    """
    Direct Yahoo chart endpoint for price/history.
...
def build_metrics(symbol, tsp_yes, sr_yes, put_call_yes, gamma_yes):
    ...
    # Yahoo/yfinance fundamentals. Some may be unavailable depending on ticker.
    ttm_pe = safe_float(yf_info.get("trailingPE"))
    forward_pe = safe_float(yf_info.get("forwardPE"))
    peg = safe_float(yf_info.get("pegRatio"))

    # Yahoo fields are usually decimal rates, e.g. 0.12 = 12%.
...
# UI
# -----------------------------
st.title("Yahoo Stock Scoring Dashboard")
...
with st.sidebar:
    ...
    st.caption("No API key or Streamlit Secrets needed.")
