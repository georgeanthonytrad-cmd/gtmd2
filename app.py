...
@st.cache_data(ttl=3600)
def get_extra_yfinance_fallbacks(symbol, current_price=np.nan):
    """
    Extra fallbacks for fields that often disappear from Yahoo info:
    - Forward P/E from analyst EPS estimates
    - Analyst target upside from analyst price target tables
    - Short % of float from shares short / float shares
    """
    result = {}
    debug = []
    ...
    try:
        ...
        # Short % of float fallback
        try:
            # These may exist even when shortPercentOfFloat is missing.
            info = {}
            try:
                info = ticker.info or {}
            except Exception:
            ...
            shares_short = first_valid(
                info.get("sharesShort"),
                info.get("sharesShortPriorMonth"),
            )
            float_shares = first_valid(
                info.get("floatShares"),
                info.get("sharesOutstanding"),
            )

            if is_num(shares_short) and is_num(float_shares) and float_shares > 0:
                result["short_float"] = (shares_short / float_shares) * 100
                debug.append("short float calculated from shares short / float shares")
        except Exception as e:
            debug.append(f"short float fallback failed: {e}")
        ...
        try:
            ...
            if isinstance(price_targets, dict):
                target = first_valid(
                    ...
                    price_targets.get("median"),
                    price_targets.get("high"),
                    price_targets.get("low"),
                )
                if is_num(target):
                    result["target_mean_price"] = target
                    debug.append("analyst target loaded from analyst_price_targets")

            # Older yfinance versions sometimes expose a DataFrame
            if not is_num(result.get("target_mean_price")):
                try:
                    pt = ticker.get_analyst_price_targets()
                    if isinstance(pt, dict):
                        target = first_valid(pt.get("mean"), pt.get("median"), pt.get("high"), pt.get("low"))
                        if is_num(target):
                            result["target_mean_price"] = target
                            debug.append("analyst target loaded from get_analyst_price_targets")
                except Exception:
                    pass

        except Exception as e:
            debug.append(f"analyst target fallback failed: {e}")

        # Forward EPS / forward P/E fallback
        try:
            forward_eps = np.nan

            # Newer yfinance: earnings_estimate DataFrame
            try:
                ee = ticker.earnings_estimate
                if ee is not None and not ee.empty:
                    # Columns vary: try current year / next year avg estimate style
                    for possible_col in ["avg", "Avg", "average"]:
                        if possible_col in ee.columns:
                            vals = pd.to_numeric(ee[possible_col], errors="coerce").dropna()
                            if len(vals) > 0:
                                # Prefer next fiscal year if present, otherwise first available.
                                forward_eps = safe_float(vals.iloc[0])
                                break
            except Exception:
                pass

            # Calendar/fiscal earnings as backup
            if not is_num(forward_eps):
                try:
                    ce = ticker.calendar
                    if isinstance(ce, pd.DataFrame):
                        for row in ["Earnings Average", "Earnings Estimate Avg", "EPS Estimate Average"]:
                            if row in ce.index:
                                vals = pd.to_numeric(ce.loc[row], errors="coerce").dropna()
                                if len(vals) > 0:
                                    forward_eps = safe_float(vals.iloc[0])
                                    break
                    elif isinstance(ce, dict):
                        forward_eps = first_valid(
                            ce.get("Earnings Average"),
                            ce.get("EPS Estimate Average"),
                            ce.get("earningsAverage"),
                        )
                except Exception:
                    pass

            # info fallback for forward EPS
            if not is_num(forward_eps):
                try:
                    info = ticker.info or {}
                    forward_eps = first_valid(info.get("forwardEps"), info.get("forwardEPS"))
                except Exception:
                    pass

            if is_num(current_price) and current_price > 0 and is_num(forward_eps) and forward_eps > 0:
                result["forward_pe"] = current_price / forward_eps
                result["forward_eps"] = forward_eps
                debug.append("forward P/E calculated from forward EPS estimate")

        except Exception as e:
            debug.append(f"forward PE fallback failed: {e}")

        return result, "; ".join(debug) if debug else "no extra yfinance fallback fields found"

    except Exception as e:
        return result, f"extra yfinance fallback failed: {e}"


def calculate_peg_fallback(ttm_pe, eps_growth, revenue_growth):
    """
    Approximate PEG when Yahoo does not provide PEG.
    Uses EPS growth first; revenue growth as backup.
    PEG convention uses growth as a whole percent, e.g. PE 20 / growth 25 = 0.8.
    """
    growth = np.nan
    if is_num(eps_growth) and eps_growth > 0:
        growth = eps_growth
    elif is_num(revenue_growth) and revenue_growth > 0:
        growth = revenue_growth

    if is_num(ttm_pe) and ttm_pe > 0 and is_num(growth) and growth > 0:
        return ttm_pe / growth
...
def build_metrics(symbol):
    ...
    extra_yf_info, extra_yf_debug = get_extra_yfinance_fallbacks(symbol, current_price)
    debug.append(f"Extra yfinance fallback: {extra_yf_debug}")
    ...
    if not is_num(peg):
        peg = calculate_peg_fallback(ttm_pe, eps_growth, revenue_growth)
    ...
    avg_target = first_valid(
        ...
        extra_yf_info.get("target_mean_price"),
    ...
    values = {
        ...
        "Forward P/E Source": "Reported" if is_num(first_valid(yf_info.get("forwardPE"), nested_raw(yahoo_summary, "defaultKeyStatistics", "forwardPE"))) else ("Calculated from EPS estimate" if is_num(extra_yf_info.get("forward_pe")) or is_num(fmp_info.get("forward_pe")) else "N/A"),
        "Forward P/E < TTM P/E": forward_pe_less_than_ttm,
        "PEG": peg,
        "PEG < 1.2": peg_under_1_2,
        "PEG Source": "Reported" if is_num(first_valid(yf_info.get("pegRatio"), nested_raw(yahoo_summary, "defaultKeyStatistics", "pegRatio"), fmp_info.get("peg"))) else ("Calculated fallback" if is_num(peg) else "N/A"),
        ...
        "Analyst Target Price": avg_target,
