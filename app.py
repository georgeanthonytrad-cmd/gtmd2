...
def get_operating_cash_flow(symbol):
    cashflow = get_cashflow_data(symbol)
    if cashflow.empty:
        return np.nan
...
def build_metrics(symbol):
    ...
    eps_growth = safe_float(yf_info.get("earningsQuarterlyGrowth"))
    eps_growth = eps_growth * 100 if is_num(eps_growth) else np.nan

    # Yahoo/yfinance revenueGrowth is most recent quarter YoY growth.
    revenue_growth = safe_float(yf_info.get("revenueGrowth"))
    revenue_growth = revenue_growth * 100 if is_num(revenue_growth) else np.nan
    ...
    short_float = short_float * 100 if is_num(short_float) else np.nan
