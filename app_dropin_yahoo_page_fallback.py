...
def get_yfinance_info(symbol):
    """
    Robust yfinance fundamentals pull.
    Tries both ticker.get_info() and ticker.info.
    """
    if yf is None:
        return {}
    ...
    try:
        ...
        try:
            ...
            if fast:
                for k in ["last_price", "regular_market_price", "market_cap"]:
                    try:
                        val = fast.get(k)
                        if val is not None:
                            info[f"fast_{k}"] = val
...
def get_yahoo_quote_summary(symbol):
    ...
    for base in ["https://query2.finance.yahoo.com", "https://query1.finance.yahoo.com"]:
        url = f"{base}/v10/finance/quoteSummary/{symbol}"
        try:
            response = requests.get(url, params=params, headers=headers, timeout=20)
            if response.status_code != 200:
            ...
            errors.append(f"{base} no result: {error}")
        except Exception as e:
            errors.append(f"{base} failed: {e}")

    return {}, " | ".join(errors)


@st.cache_data(ttl=3600)
def get_yahoo_page_store(symbol):
    """
    Last-resort Yahoo fallback.
    It reads the public Yahoo quote/statistics page and extracts Yahoo's embedded
    QuoteSummaryStore JSON. This often works when yfinance info is blank.
    """
    headers = {
    ...
    for url in urls:
        try:
            ...
            # Yahoo embeds a JSON object in: root.App.main = {...};
            match = re.search(r"root\.App\.main\s*=\s*(\{.*?\})\s*;\s*\n", text, flags=re.S)
            if not match:
                # backup: search more broadly
                match = re.search(r"root\.App\.main\s*=\s*(\{.*?\})\s*;\s*</script>", text, flags=re.S)

            if not match:
                errors.append(f"{url} no root.App.main")
                continue
            ...
            if isinstance(qss, dict):
                combined_store.update(qss)

            # Some pages expose QuoteTimeSeriesStore or QuotePageStore; keep if useful later.
            for store_name in ["QuotePageStore", "QuoteTimeSeriesStore"]:
                store = stores.get(store_name, {})
                if isinstance(store, dict):
                    combined_store[store_name] = store
...
def build_metrics(symbol):
    ...
    yahoo_page_store, yahoo_page_error = get_yahoo_page_store(symbol)
    ...
    if yahoo_page_store:
        debug.append("Yahoo public page fallback loaded")
    else:
        debug.append(f"Yahoo public page fallback not loaded: {yahoo_page_error}")
    ...
    if not is_num(current_price):
        current_price = first_valid(
            ...
            nested_raw(yahoo_page_store, "price", "regularMarketPrice"),
        )
    ...
    # Fundamentals: try yfinance, quoteSummary, then Yahoo public page store.
    ttm_pe = first_valid(
        ...
        nested_raw(yahoo_page_store, "summaryDetail", "trailingPE"),
        nested_raw(yahoo_page_store, "defaultKeyStatistics", "trailingPE"),
    )
    ...
    forward_pe = first_valid(
        ...
        nested_raw(yahoo_page_store, "defaultKeyStatistics", "forwardPE"),
        nested_raw(yahoo_page_store, "summaryDetail", "forwardPE"),
    )
    ...
    peg = first_valid(
        ...
        nested_raw(yahoo_page_store, "defaultKeyStatistics", "pegRatio"),
        nested_raw(yahoo_page_store, "defaultKeyStatistics", "trailingPegRatio"),
    )
    ...
    eps_growth_raw = first_valid(
        ...
        nested_raw(yahoo_page_store, "financialData", "earningsGrowth"),
    )
    eps_growth = eps_growth_raw * 100 if is_num(eps_growth_raw) else np.nan
    ...
    revenue_growth_raw = first_valid(
        ...
        nested_raw(yahoo_page_store, "financialData", "revenueGrowth"),
    )
    revenue_growth = revenue_growth_raw * 100 if is_num(revenue_growth_raw) else np.nan
    ...
    avg_target = first_valid(
        ...
        nested_raw(yahoo_page_store, "financialData", "targetMeanPrice"),
        nested_raw(yahoo_page_store, "financialData", "targetMedianPrice"),
    )
    ...
    # Extra PE fallback: calculate P/E from EPS values when Yahoo gives EPS but not P/E.
    trailing_eps = first_valid(
        yf_info.get("trailingEps"),
        nested_raw(yahoo_summary, "defaultKeyStatistics", "trailingEps"),
        nested_raw(yahoo_page_store, "defaultKeyStatistics", "trailingEps"),
    )
    forward_eps = first_valid(
        yf_info.get("forwardEps"),
        nested_raw(yahoo_summary, "defaultKeyStatistics", "forwardEps"),
        nested_raw(yahoo_page_store, "defaultKeyStatistics", "forwardEps"),
    )

    if not is_num(ttm_pe) and is_num(current_price) and is_num(trailing_eps) and trailing_eps > 0:
        ttm_pe = current_price / trailing_eps

    if not is_num(forward_pe) and is_num(current_price) and is_num(forward_eps) and forward_eps > 0:
    ...
    short_float_raw = first_valid(
        ...
        nested_raw(yahoo_page_store, "defaultKeyStatistics", "shortPercentOfFloat"),
    )
    short_float = short_float_raw * 100 if is_num(short_float_raw) else np.nan

    # Gross margin (useful for pre-profit companies instead of net margin)
    gross_margin_raw = first_valid(
        yf_info.get("grossMargins"),
        nested_raw(yahoo_summary, "financialData", "grossMargins"),
        nested_raw(yahoo_page_store, "financialData", "grossMargins"),
    )
    gross_margin = gross_margin_raw * 100 if is_num(gross_margin_raw) else np.nan

    # Earnings surprise % from most recent quarter
    earnings_surprise_raw = first_valid(
        yf_info.get("earningsSurprisePercent"),
        nested_raw(yahoo_summary, "defaultKeyStatistics", "earningsSurprisePercent"),
        nested_raw(yahoo_page_store, "defaultKeyStatistics", "earningsSurprisePercent"),
    )
    # yfinance also exposes this via .earnings_history — try as fallback
    if not is_num(earnings_surprise_raw) and yf is not None:
        try:
            ticker_obj = yf.Ticker(symbol)
            eh = ticker_obj.earnings_history
            if eh is not None and not eh.empty and "surprisePercent" in eh.columns:
                last_surprise = pd.to_numeric(eh["surprisePercent"], errors="coerce").dropna()
                if not last_surprise.empty:
                    earnings_surprise_raw = float(last_surprise.iloc[-1])
        except Exception:
            pass
    earnings_surprise = earnings_surprise_raw * 100 if is_num(earnings_surprise_raw) and abs(safe_float(earnings_surprise_raw, 0)) <= 1 else safe_float(earnings_surprise_raw)

    total_cash = first_valid(
        yf_info.get("totalCash"),
        nested_raw(yahoo_summary, "financialData", "totalCash"),
        nested_raw(yahoo_page_store, "financialData", "totalCash"),
    )
    ...
    pts = (
        10 if is_num(revenue_growth) and revenue_growth > 20 else
        7 if is_num(revenue_growth) and revenue_growth > 10 else
        4 if is_num(revenue_growth) and revenue_growth > 0 else
        0
    )
    score += pts
    details.append(("Quarterly revenue growth", pts, revenue_growth, "Max 10"))
    ...
    # Meaningful for both profitable and pre-profit companies
    pts = (
        7 if is_num(gross_margin) and gross_margin >= 60 else
        5 if is_num(gross_margin) and gross_margin >= 40 else
        3 if is_num(gross_margin) and gross_margin >= 20 else
        1 if is_num(gross_margin) and gross_margin >= 0 else
        0
    )
    score += pts
    details.append(("Gross Margin %", pts, gross_margin, "Max 7"))

    # EARNINGS SURPRISE — 5 POINTS
    # Did the company beat estimates last quarter?
    pts = (
        5 if is_num(earnings_surprise) and earnings_surprise > 5 else
        3 if is_num(earnings_surprise) and earnings_surprise > 0 else
        1 if is_num(earnings_surprise) and earnings_surprise > -5 else
        0
    )
    score += pts
    details.append(("Earnings Surprise %", pts, earnings_surprise, "Max 5"))
    ...
    pts = score_range(change_5d, [
        (lambda x: 2 <= x <= 8, 5),   # healthy positive momentum
        (lambda x: 0 <= x < 2, 3),    # mild positive
        (lambda x: x > 8, 2),         # very extended — still positive but trim risk
        (lambda x: -3 <= x < 0, 1),   # slight dip, not alarming
        (lambda x: x < -3, 0),        # meaningful drop
    ])
    score += pts
    ...
    pts = score_range(rsi, [
        (lambda x: 50 <= x <= 65, 6),   # ideal: bullish momentum, not extended
        (lambda x: 40 <= x < 50, 4),    # constructive, building
        (lambda x: 65 < x <= 70, 3),    # getting warm
        (lambda x: 30 <= x < 40, 2),    # oversold recovery possible
        (lambda x: x > 70, 0),          # overbought — also a trim signal
        (lambda x: x < 30, 0),          # oversold — avoid catching falling knives
    ])
    score += pts
    ...
    # SHORT INTEREST — 8 POINTS (low short % = healthy, high = risk)
    pts = (
        8 if is_num(short_float) and short_float < 3 else
        6 if is_num(short_float) and short_float < 5 else
        4 if is_num(short_float) and short_float < 10 else
        2 if is_num(short_float) and short_float < 15 else
        0  # 15%+ short float is a red flag
    )
    score += pts
    details.append(("Short % of Float", pts, short_float, "Max 8"))
...
def format_scanner_df(df):
    columns = [
        ...
        "Short % of Float", "Gross Margin %", "Earnings Surprise %", "Cash Runway Months",
    ]
