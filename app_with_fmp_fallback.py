...
@st.cache_data(ttl=3600)
def fmp_get_json(endpoint, symbol):
    """
    ...
    try:
        ...
        if r.status_code != 200:
            return None, f"FMP HTTP {r.status_code}: {r.text[:120]}"
...
def get_fmp_fundamentals(symbol):
    """
    Pulls missing fundamentals from FMP when Yahoo/yfinance returns N/A.
    """
    out = {}
    errors = []

    quote, err = fmp_get_json("quote", symbol)
    ...
    ratios, err = fmp_get_json("ratios-ttm", symbol)
    if ratios and isinstance(ratios, list):
        ...
        gm = first_valid(r.get("grossProfitMarginTTM"), r.get("grossProfitMargin"))
        out["gross_margin"] = gm * 100 if is_num(gm) and abs(gm) <= 1 else gm
    else:
        errors.append(f"ratios-ttm: {err}")

    key_metrics, err = fmp_get_json("key-metrics-ttm", symbol)
    if key_metrics and isinstance(key_metrics, list):
        km = key_metrics[0]
        out["forward_pe"] = first_valid(km.get("forwardPE"), km.get("forwardPe"))
    else:
        errors.append(f"key-metrics-ttm: {err}")

    # Income statement quarterly for quarterly revenue growth and EPS growth fallback
    api_key = get_fmp_key()
    if api_key:
        try:
            url = f"https://financialmodelingprep.com/api/v3/income-statement/{symbol}"
            r = requests.get(url, params={"period": "quarter", "limit": 8, "apikey": api_key}, timeout=20)
            if r.status_code == 200:
                inc = r.json()
                ...
                errors.append(f"income-statement: HTTP {r.status_code}")
        except Exception as e:
            errors.append(f"income-statement: {e}")

        # Balance sheet quarterly for cash
        try:
            url = f"https://financialmodelingprep.com/api/v3/balance-sheet-statement/{symbol}"
            r = requests.get(url, params={"period": "quarter", "limit": 1, "apikey": api_key}, timeout=20)
            if r.status_code == 200:
                bs = r.json()
                ...
                errors.append(f"balance-sheet: HTTP {r.status_code}")
        except Exception as e:
            errors.append(f"balance-sheet: {e}")

        # Cash flow quarterly annualized fallback
        try:
            url = f"https://financialmodelingprep.com/api/v3/cash-flow-statement/{symbol}"
            r = requests.get(url, params={"period": "quarter", "limit": 4, "apikey": api_key}, timeout=20)
            if r.status_code == 200:
                cf = r.json()
                ...
                errors.append(f"cash-flow: HTTP {r.status_code}")
        except Exception as e:
            errors.append(f"cash-flow: {e}")

        # Analyst estimates can be spotty but useful for forward EPS
        try:
            url = f"https://financialmodelingprep.com/api/v3/analyst-estimates/{symbol}"
            r = requests.get(url, params={"period": "annual", "limit": 2, "apikey": api_key}, timeout=20)
            if r.status_code == 200:
                est = r.json()
                if isinstance(est, list) and est:
                    e = est[0]
                    estimated_eps = first_valid(e.get("estimatedEpsAvg"), e.get("estimatedEpsHigh"), e.get("estimatedEpsLow"))
                    price = first_valid(out.get("price"))
                    if is_num(price) and is_num(estimated_eps) and estimated_eps > 0:
                        out["forward_pe"] = price / estimated_eps
            else:
                errors.append(f"analyst-estimates: HTTP {r.status_code}")
        except Exception as e:
            errors.append(f"analyst-estimates: {e}")
...
def build_metrics(symbol):
    ...
    eps_growth = eps_growth_raw * 100 if is_num(eps_growth_raw) else fmp_info.get("eps_growth")
    ...
    revenue_growth = revenue_growth_raw * 100 if is_num(revenue_growth_raw) else fmp_info.get("revenue_growth")
    ...
    gross_margin = gross_margin_raw * 100 if is_num(gross_margin_raw) else fmp_info.get("gross_margin")
    ...
    values = {
        ...
        "Data Source": "Yahoo direct chart + yfinance + Yahoo quoteSummary + optional FMP fallback",
    }
...
st.sidebar.caption("Optional: add FMP_API_KEY to Streamlit Secrets for reliable fundamentals.")
